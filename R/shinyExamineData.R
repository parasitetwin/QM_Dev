...
examineDataServer<-function(id,r){
  moduleServer(
    ...
    function(input, output, session){
      ...
      ####Rendering plot areas based on whether data is present in DB or not####
      output$displayGraphs <- renderUI({
        if(!is.null(r$examineData$tablePresent)){
          fluidRow(
            column(
              width=6,
              tabBox(
      ...
      output$plotStatus<-renderPlotly({
        ...
        req(r$examineData$tablePresent)
      ...
      observe({
        ...
        isolate({
          if(file.exists(r$examineData$config$dbName)){
            ...
            message(r$examineData$config$sampleMatrix)

            #Reading LaMa related data
            if(r$examineData$projName != "All samples"){

              s1 <- sprintf("SELECT i.name, i.nLMs, i.nPeaks, i.TIC, i.IPOscore, i.batchWeek, i.sampID FROM [%s] i WHERE i.chromPol='%s' AND i.projName='%s' AND i.type='%s'",
              ...
              s1 <- sprintf("SELECT i.name, i.nLMs, i.nPeaks, i.TIC, i.IPOscore, i.batchWeek, i.sampID FROM [%s] i WHERE i.chromPol='%s' AND i.type='%s'",
                            paste0("QTableDistinct_",r$examineData$config$sampleMatrix),
                            r$examineData$chromPol,
                            r$examineData$sampType)
            ...
            s3 <- sprintf("SELECT name FROM sqlite_master WHERE type='table' AND name='%s'",
                          paste0("QTable_",r$examineData$config$sampleMatrix))
            ...
            if(nrow(r$examineData$tablePresent) == 0){
              r$examineData$tablePresent <- NULL
            }

            message(paste0("Examine data: ", r$examineData$tablePresent))
          }
        })
      })
